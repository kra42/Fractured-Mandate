...
func _ready():
	unit_class = "WARRIOR"
	# Beginner Friendly Stats: Good HP, decent speed, reliable damage.
	max_hp = 50
	current_hp = 50
	max_qi = 6
	current_qi = 1 
...
func get_skill_info(type: String) -> Dictionary:
	...
	if type == "PASSIVE":
		...
		info.desc = "Attacks 7 times. Chains on kill. Each hit triggers Passive and on-hit effects."
		# 80% Scaling
		var per_hit = ceil(attack_power * 0.8)
		var total = per_hit * 7
